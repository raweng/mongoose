...
Model.prototype.save = function save (fn) {
  var promise = new Promise(fn)
    , mcomplete = handleSave(promise, this)
    , options = {}
  ...
  // Overriding the default complete and adding our own here
  var model = this;
  var complete = function() {
    var origArgs = arguments;

    // if we have any post saves, we complete them first
    var postSaves = model.schema.postSaves.length ? 
      model.schema.postSaves :
      [function(obj, next) {return next();}]

    var index = 0;
    var process = function() {
      return postSaves[index].apply(model, [model, next]);
    }
    var next = function() {
      if (++index === postSaves.length)
        return mcomplete.apply(model, origArgs); // postSaves are done
...
Model.prototype.$__version = function (where, delta) {
  ...
  if (true === where) {
    // this is an insert
    if (key) this.setValue(key, delta[key] = 1);
    return;
  }
